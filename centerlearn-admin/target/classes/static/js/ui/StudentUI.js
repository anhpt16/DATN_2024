...
const studentUI = {
    ...
    renderTable: (accounts) => {
        studentUI.el.tbodyEl.empty();
        if (accounts.length === 0) {
            studentUI.el.tbodyEl.append('<tr><td colspan="8">No account available</td></tr>');
            return;
        }
        accounts.forEach(function(account) {
            let row = '<tr class="fade-in" data-id="' + account.id + '">' +
                ...
                '</td>' +
            '</tr>';
            studentUI.el.tbodyEl.append(row);
        })
        setTimeout(function() {
            studentUI.el.tbodyEl.find('.fade-in').addClass('show');
          }, 100);
    },
    renderEditModal: () => {
